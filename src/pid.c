...
void stable_mode()
{
	...
	pid_params x = {KC, TAU_I, TAU_D, TAU_F, 0, 0, x_cord, 0, 0, 0, 0, 0};  //initialise PID parameters for x axis
	...
	pid_params y = {-KC, -TAU_I, -TAU_D, -TAU_F, 0, 0, y_cord, 0, 0, 0, 0, 0}; //initialise PID paramaters for y axis
	gettimeofday(&tim, NULL);
	...
	while(!next_mode)
	{
		...
		x.pos_curr = x_cord;  //current ball position is equal to the coordinates read from the touchscreen
		...
		y.pos_curr = y_cord;  //current ball position is equal to the coordinates read from the touchscreen
